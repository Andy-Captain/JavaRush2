...
public class Solution {
    ...
    public  <T extends Item> T getProxy(Class<T> itemClass, Class<?> ... interfaces)
        {
            Class[] clazz = new Class[interfaces.length+1];
            for ( int i = 0; i < interfaces.length; i++) {
                 clazz[i]=interfaces[i];
            }
             clazz[clazz.length-1] = itemClass;
            return (T) Proxy.newProxyInstance(itemClass.getClassLoader(), clazz, new ItemInvocationHandler());
        }
